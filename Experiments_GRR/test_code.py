...
def kRR_test(true_list,num1,epsilon_list,test_time):
    ...
    for i in range(test_time):
        result1 = drp.kRR_reuse(true_list, num1, epsilon_list)

        err_naive += result1[0]
        err_baseline += result1[1]
        err_best += drp.kRR(true_list, num1, sum(epsilon_list))[0]

    print(err_naive/test_time,"\t",err_baseline/test_time,"\t",err_best/test_time)
    return 0


def kRR_partition_test(true_list,epsilon,n,test_time):
    ...
    for t in range(test_time):
        ...
        for i in range(1, n + 1):
            ...
            err_temp = error_cal(true_list,[(int(value_2*n/i)) for value_2 in drp.kRR_partition(shuffled_list, epsilon)])
    ...
    for t in range(test_time):
        ...
        for i in range(1, n + 1):
            ...
            for j in range(1, i + 1):
                ...
                new_es=drp.kRR(true_list,sum(true_list),epsilon_temp)[1]

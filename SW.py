...
def sw_solve(M_1, M_2):
    ...
    constraints = [
        B >= 10**(-5),
        cp.sum(B, axis=1) == 1  # 每行和为1
    ]

    # 目标函数：最小化 ||A @ B - C||_F^2
    objective = cp.Minimize(cp.sum_squares(M_1 @ B - M_2))
...
for i in range(test_time):
    list_2 = sw_list(v, d, b2, e2, n)
    list_1 = sw_list(v, d, b1, e1, n)

    list_12 = sw_reuse(list_1, M_12, d, b2)

    list_20 = sw_list(v, d, b2, e2, n)
